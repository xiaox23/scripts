...
def read_and_plot(folder_path):
    ...
    # 绘制 P1 最大值
    plt.plot(timestamps, P1_max_values, label="P1 Max", color="red", linewidth=2)
    ...
    # 设置更密集的横坐标刻度
    plt.xticks(
        ticks=np.linspace(x_min, x_max, 100, dtype=int),  # 现在的 10 倍密度
        rotation=45,
        fontsize=10
    )
    ...
    # 设置标题
    plt.title("Variation of pos, force, and P1 Max with Timestamps", fontsize=16)
