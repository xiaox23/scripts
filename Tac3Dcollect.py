...
# 将数据保存到指定文件夹中，文件名根据已有文件数量自动命名。
def save_to_pickle(data, folder_path = 'result/test'):
    try:
        ...
        # 自动生成文件名
        file_name = f"{file_index:04d}.pkl"
...
if __name__ == "__main__":
    ...
    def HandRecvCallback(client: DexHandClient):
        ...
        if info._frame_cnt % 100 == 0:
            ...
            # 调用保存函数
            save_to_pickle(data, folder_path = 'result/test3')
