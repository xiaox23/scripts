...
# TODO 等键盘按下"z"之后执行下面的命令
    ...
    while True:
        ...
        if key == "z":
            ...
            # 获取当前时间戳（毫秒格式）
            current_time = 1000 * round(time.time(), 3)  # Unix 时间戳，单位为毫秒

            try:
                with open("execution_time.txt", "a") as file:  # 使用 "a" 模式以追加方式写入
                    file.write(f"{current_time}\n")
            except Exception as e:
                print(f"Failed to save timestamp: {e}")
                break

            print("Time saved to execution_time.txt")
            break

# TODO 等键盘按下"f"之后执行下面的命令
...
# python scripts/E01TacData.py --folder_path data_save/tac_data