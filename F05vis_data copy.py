...
def process_and_plot_cal_data(input_base_folder, output_base_folder, x_range=(-50, 50), y_range=(-50, 50)):
    """
    读取 cal_data 文件夹中的数据，绘制 PP1 和 PP1ref 以及 PP2 和 PP2ref 的二维图像。
    Args:
    ...
    for exp_number in sorted(os.listdir(input_base_folder)):
        ...
        for file_name in pickle_files:
            ...
            if "PP1" not in tac_data or "PP1ref" not in tac_data or "PP2" not in tac_data or "PP2ref" not in tac_data:
                print(f"PP1, PP1ref, PP2, or PP2ref key not found in file: {file_path}")
                continue

            PP1 = tac_data["PP1"]
            PP1ref = tac_data["PP1ref"]
            PP2 = tac_data["PP2"]
            PP2ref = tac_data["PP2ref"]

            # 绘制并保存图像
            plot_title = f"2D Plot of PP1/PP1ref and PP2/PP2ref (File: {file_name})"
